...
const initDb = async () => {
    try {
        await pool.query(`CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, username VARCHAR(255) UNIQUE NOT NULL, email VARCHAR(255) UNIQUE NOT NULL, password_hash VARCHAR(255) NOT NULL, otp VARCHAR(10), otp_expiry TIMESTAMP)`);
...
app.post('/api/signup', async (req, res) => {
    ...
    if (!username || !email || !password || password.length < 8) return res.status(400).send("Invalid input. Password must be at least 8 characters.");
    try {
        ...
        if (err.code === '23505') return res.status(409).send("Username or Email already exists.");
...
app.post('/api/verify-otp', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT * FROM users WHERE email = $1`, [email.toLowerCase()]);
        if (result.rows.length === 0) return res.status(401).send("Invalid request.");
        const user = result.rows[0];
        if (user.otp === otp && new Date() < new Date(user.otp_expiry)) {
            await pool.query(`UPDATE users SET otp = NULL, otp_expiry = NULL WHERE email = $1`, [email.toLowerCase()]);
            res.status(200).json({ username: user.username, email: user.email });
        } else {
...
app.post('/api/change-password', async (req, res) => {
    ...
    if (!newPassword || newPassword.length < 8) return res.status(400).send("Password must be at least 8 characters.");
    try {
        const hash = await bcrypt.hash(newPassword, 10);
        await pool.query(`UPDATE users SET password_hash = $1 WHERE username = $2`, [hash, username.toLowerCase()]);
